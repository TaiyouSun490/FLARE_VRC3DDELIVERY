...
namespace AvatarCatalog.Remote
{
    ...
    public sealed class SimpleGlbRuntimeLoader01 : UdonSharpBehaviour
    {
        ...
        private bool View(int index, int elementBytes, out int start, out int length, out int stride)
        {
            ...
            bool hasStride = TryGetInt(view, "byteStride", out int foundStride);
            if (hasStride) stride = foundStride;
            if (offset < 0 || length < elementBytes || stride < elementBytes || stride > 252 ||
                hasStride && (stride & 3) != 0 || (long)offset + length > _binLength) return false;
